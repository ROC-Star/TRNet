...
def smt_t(pretrained=False, **kwargs):
    ...
    if pretrained:
        print("=============Loading smt_tiny=====================")
        model.load_state_dict(torch.load('./pre/smt_tiny.pth')['model'])
        # pretrain_state_dict = torch.load('/media/omnisky/data/rp/NewSOD/pre/smt_tiny.pth')
        # model.load_state_dict(pretrain_state_dict['model'])
        # model = free_model(model, pretrain_state_dict)
    return model


# freeze pretrain model params
def free_model(model, to_freeze_dict):
...
def smt_s(pretrained=False, **kwargs):
    ...
    if pretrained:
        # print("=============Loading smt_l=====================")
        model.load_state_dict(torch.load('/media/omnisky/data/rp/NewSOD/pre/smt_small.pth')['model'])

    return model


def smt_b(pretrained=True, **kwargs):
    model = SMT(
        embed_dims=[64, 128, 256, 512], ca_num_heads=[4, 4, 4, -1], sa_num_heads=[-1, -1, 8, 16],
        mlp_ratios=[8, 6, 4, 2],
    ...
    if pretrained:
        # print("=============Loading smt_l=====================")
        model.load_state_dict(torch.load('/media/omnisky/data/rp/NewSOD/pre/smt_base.pth')['model'])

    return model


def smt_l(pretrained=True, **kwargs):
    model = SMT(
    ...
    if pretrained:
        # print("=============Loading smt_l=====================")
        model.load_state_dict(torch.load('/media/omnisky/data/rp/NewSOD/pre/smt_large_22k.pth')['model'])
...
if __name__ == '__main__':
    ...
    ### ptflops cal ###
    flops_count, params_count = get_model_complexity_info(model, (3, 384, 384), as_strings=True,
